...
void print_circle_to_svg1(FILE *svgFile, _Tree tree){
	if(is_null(tree)){
		return;
	}
	print_circle_to_svg1(svgFile, get_kd_right(tree));
	print_circle_to_svg1(svgFile, get_kd_left(tree));
	_Circle circle = get_info(tree);
	int state = get_circle_state(circle);
	if(state == 1){
		print_circle_animated(svgFile, circle);
	}
}
...
void fill_svg(_Tree rect_tree, _Tree circle_tree, FILE *svgFile){
	...
	print_circle_to_svg1(svgFile, circle_tree);
